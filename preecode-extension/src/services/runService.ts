...
export async function runActiveFile(): Promise<RunResult> {
    ...
    try {
        ...
        // Reuse existing "preecode" terminal if open, otherwise create one.
        // This mirrors how Code Runner reuses its output channel.
        let terminal = vscode.window.terminals.find(t => t.name === "preecode");
        if (!terminal) {
            terminal = vscode.window.createTerminal("preecode");
        }
